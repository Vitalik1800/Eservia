...
import com.eservia.glide.load.engine.DecodeJob.DiskCacheProvider;
...
final class DecodeHelper<Transcode> {
  ...
  boolean isSourceKey(Key key) throws ClassNotFoundException {
  ...
  List<LoadData<?>> getLoadData() throws ClassNotFoundException {
    if (!isLoadDataSet) {
      isLoadDataSet = true;
      loadData.clear();
  ...
  List<Key> getCacheKeys() throws ClassNotFoundException {
    if (!isCacheKeysSet) {
      isCacheKeysSet = true;
      cacheKeys.clear();
      List<LoadData<?>> loadData = getLoadData();
